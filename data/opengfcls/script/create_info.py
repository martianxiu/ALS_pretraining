...
parser = argparse.ArgumentParser(description='generate sample pickle per region')
parser.add_argument('-i', '--input_dir', type=str, required=True, help='The path to the input log file.')
# parser.add_argument('-o', '--output_dir', type=str, required=True, help='The path to the output file for extracted lines.')
...
root_training = join(input_dir, "train_cls_w100_s50")
training_tiles = get_laz_file_paths(root_training)

root_test = join(input_dir, "test_cls_w100_s100")
test_tiles = get_laz_file_paths(root_test)

root_val = join(input_dir, "val_cls_w100_s100")
val_tiles = get_laz_file_paths(root_val)

create_info("train_cls_w100_s50", training_tiles, root_training)
create_info("test_cls_w100_s100", test_tiles, root_test)
create_info("val_cls_w100_s100", val_tiles, root_val)