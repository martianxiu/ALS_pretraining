...
class ALSBEVMAEDataset(DatasetTemplateALSBEVMAE):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, root_path=root_path, logger=logger
        )
        self.data_path = self.root_path / self.dataset_cfg.PROCESSED_DATA_TAG
        self.num_sample_per_region = self.dataset_cfg.NUM_SAMPLE_PER_REGION
    ...
    #     self.logger.info('Loading ALS dataset')
    #     als_infos = []
    #     num_skipped_infos = 0
    #     for k in range(len(self.region_list)):
    #         region_name = os.path.splitext(self.region_list[k])[0]
    #         info_path = self.data_path / region_name / (f"{region_name}_{self.num_sample_per_region}.pkl")
    #         self.logger.info(info_path)
    #         if not info_path.exists():
    #             num_skipped_infos += 1
    #             self.logger.info(f"skipped {info_path}")
    #             continue
    ...
    #     self.logger.info('Total samples for ALS dataset: %d' % (len(als_infos)))

    def include_als_data(self, mode):
        self.logger.info('Loading ALS dataset')
        als_infos = []
        num_skipped_infos = 0
        info_path = self.data_path / (f"all_{self.num_sample_per_region}.pkl")
        with open(info_path, 'rb') as f:
            infos = pickle.load(f)
            als_infos.extend(infos)
    ...
    def load_data_to_shared_memory(self):
        ...
        # used_indices = list(set(range(cur_rank, len(all_infos), num_gpus)))
        # used_indices = np.sort(used_indices)
        # self.logger.info(used_indices)
        # remaining_infos = [self.infos[i] for i in range(len(self.infos)) if i not in used_indices]
        # remaining_infos = self.infos[self.shared_memory_file_limit:]
        # self.remaining_infos = remaining_infos
    ...
    def get_lidar(self, region_name, sample_idx):
        def process_laz(laz_file_path):
            ...
            valid_indices = ~np.isnan(xyz).any(axis=1) & ~np.isnan(intensity) & ~np.isnan(return_number) & ~np.isnan(num_returns)
            # valid_indices = ~np.isnan(xyz).any(axis=1) & ~np.isnan(intensity) & ~np.isnan(return_number) & ~np.isnan(num_returns) & (num_returns != 0)
            ...
            return np.concatenate([xyz, intensity, return_number, num_returns], 1)

        lidar_file = self.data_path / region_name / f"tile_{sample_idx}.laz" # ...tile_x.laz
        points_all = process_laz(lidar_file)  # (N, 7): [x, y, z, intensity, return_num, num_return]
    ...
    def __getitem__(self, index):
        ...
        # if 'annos' in info: # check if there is any labek 
        #     annos = info['annos']
        ...
        data_dict.pop('shuffle_idx')
        data_dict.pop('crop_indices')
