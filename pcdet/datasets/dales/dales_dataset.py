...
class DALESDataset(DatasetTemplateALSBEVMAE):
    ...
    def include_als_data(self, mode):
        self.logger.info('Loading DALES dataset')
        als_infos = []
        num_skipped_infos = 0
        ...
        self.logger.info('Total samples for DALES dataset: %d' % (len(als_infos)))
    ...
    def get_lidar(self, region_name, sample_idx):
        def process_laz(laz_file_path):
            ...
            # set the unlabeled class label as 255.
            seg = np.array(seg).astype(int)
            seg -= 1 # unlabeled class is 0. 
            seg[seg==-1] = 255
            return np.concatenate([xyz, intensity, return_number, num_returns, return_ratio, seg], 1)

        lidar_file = self.data_path / f"{sample_idx}" 
        points_all = process_laz(lidar_file) 
    ...
    def __getitem__(self, index):
        ...
        index = index % len(self.infos) # loop
        ...
        if not self.pretrain_mode:
            seg = seg[shuffle_idx] if shuffle_idx is not None else seg
            seg = seg[crop_idx] if crop_idx is not None else seg
            seg = seg[random_sample_idx] if random_sample_idx is not None else seg
            if inverse_mapping is not None:
                valid_mask = inverse_mapping != -1
                seg = seg[valid_mask]
            
            data_dict['point_seg_labels'] = seg # labels after pillar crop
