...
class PointFeatureEncoder(object):
    ...
    def num_point_features(self):
        num_point_features = self.point_encoding_config.get('input_channels', None) # this will be used when #features != input channels
        if num_point_features is None:
            num_point_features = getattr(self, self.point_encoding_config.encoding_type)(points=None)
    ...
    def absolute_coordinates_encoding(self, points=None): # always use abs coords for features
        if points is None:
            num_output_features = len(self.used_feature_list)
            if 'labels' in self.used_feature_list:
