...
class PureForestDataset(DatasetTemplateALSBEVMAE):
    ...
    def include_als_data(self, mode):
        self.logger.info('Loading PureForest dataset')
        als_infos = []
        num_skipped_infos = 0
        for k in range(len(self.region_list)):
            ...
            # latin_name = region_name.split('-')[1] # the latin name of tree species 
            # # latin_name = "Pinus_pinaster"
    ...
    def get_lidar(self, region_name, sample_idx):
        def process_laz(laz_file_path):
            ...
            seg = np.array(point_data.classification) # this is not the label
