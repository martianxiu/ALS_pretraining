...
class BaseBEVBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        if len(upsample_strides) > num_levels: # assume the last upsampling upsamples beyond the input 
            self.deblocks.append(nn.Sequential(
    ...
    def forward(self, data_dict):
        ...
        Returns:
        """
        spatial_features = data_dict['spatial_features'] # torch.Size([2, 64, 468, 468])
        ups = []
        ret_dict = {}
        x = spatial_features
        for i in range(len(self.blocks)):
            ...
            stride = int(spatial_features.shape[2] / x.shape[2]) # original / current featue shape 
            ret_dict['spatial_features_%dx' % stride] = x
            if len(self.deblocks) > 0: # per layer upsampling instead of UNet like continuous upsampling. 
                ups.append(self.deblocks[i](x))
            else:
                ups.append(x)

        if len(ups) > 1: # concatenate all per-layer upsampled feature maps 
        ...
        data_dict['spatial_features_2d'] = x # torch.Size([2, 384, 468, 468])
