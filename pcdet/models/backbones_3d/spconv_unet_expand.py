...
class VoxelResBackBone8xUNet(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        self.sparse_shape = grid_size[::-1] + [1, 0, 0]
        ...
        self.conv_up_m5 = block(256, 128, 3, norm_fn=norm_fn, padding=1, indice_key='subm5')
        self.inv_conv5 = block(128, 128, kernel_size=(3, 1, 1), norm_fn=norm_fn, indice_key='spconv_down2', conv_type='inverseconv')

        # [400, 352, 11] <- [200, 176, 5]
        # x_conv4, above
        self.conv_up_t4 = SparseBasicBlock(128, 128, indice_key='res4', norm_fn=norm_fn)
        self.conv_up_m4 = block(256, 128, 3, norm_fn=norm_fn, padding=1, indice_key='res4')
        self.inv_conv4 = block(128, 64, 3, norm_fn=norm_fn, indice_key='spconv4', conv_type='inverseconv')

        # [800, 704, 21] <- [400, 352, 11]
        # x_conv3, above
        self.conv_up_t3 = SparseBasicBlock(64, 64, indice_key='res3', norm_fn=norm_fn)
        self.conv_up_m3 = block(128, 64, 3, norm_fn=norm_fn, padding=1, indice_key='res3')
        self.inv_conv3 = block(64, 32, 3, norm_fn=norm_fn, indice_key='spconv3', conv_type='inverseconv')

        # [1600, 1408, 41] <- [800, 704, 21]
        # x_conv2, above
        self.conv_up_t2 = SparseBasicBlock(32, 32, indice_key='res2', norm_fn=norm_fn)
        self.conv_up_m2 = block(64, 32, 3, norm_fn=norm_fn, indice_key='res2')
        self.inv_conv2 = block(32, 16, 3, norm_fn=norm_fn, indice_key='spconv2', conv_type='inverseconv')

        # [1600, 1408, 41] <- [1600, 1408, 41]
        self.conv_up_t1 = SparseBasicBlock(16, 16, indice_key='res1', norm_fn=norm_fn)
        self.conv_up_m1 = block(32, 16, 3, norm_fn=norm_fn, indice_key='res1')
...
class VoxelResBackBone8xUNet_expand(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, voxel_size, point_cloud_range, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        self.sparse_shape = grid_size[::-1] + [1, 0, 0]
        ...
        # conv_out, conv_out 
        self.out_inv5 = 128
        self.conv_up_t5 = SparseBasicBlock(self.out_last, self.out_last, indice_key='subm5', norm_fn=norm_fn)
        self.conv_up_m5 = block(self.out_last + self.out_last, self.out_last, 3, norm_fn=norm_fn, padding=1, indice_key='subm5')
        self.inv_conv5 = block(self.out_last, self.out_inv5, kernel_size=(3, 1, 1), norm_fn=norm_fn, indice_key='spconv_down2', conv_type='inverseconv')
        # self.conv_up_t5 = SparseBasicBlock(128, 128, indice_key='subm5', norm_fn=norm_fn)
        # self.conv_up_m5 = block(256, 128, 3, norm_fn=norm_fn, padding=1, indice_key='subm5')
        # self.inv_conv5 = block(128, 128, kernel_size=(3, 1, 1), norm_fn=norm_fn, indice_key='spconv_down2', conv_type='inverseconv')
        ...
        # x_conv4, above
        self.out_inv4 = 64
        self.conv_up_t4 = SparseBasicBlock(self.out_4, self.out_4, indice_key='res4', norm_fn=norm_fn) # applied to level 4
        self.conv_up_m4 = block(self.out_4 + self.out_inv5, self.out_inv4, 3, norm_fn=norm_fn, padding=1, indice_key='res4') # applied to combined feat
        self.inv_conv4 = block(self.out_inv4, self.out_inv4, 3, norm_fn=norm_fn, indice_key='spconv4', conv_type='inverseconv')

        # [800, 704, 21] <- [400, 352, 11]
        # x_conv3, above
        self.out_inv3 = 32
        self.conv_up_t3 = SparseBasicBlock(self.out_3, self.out_3, indice_key='res3', norm_fn=norm_fn)
        self.conv_up_m3 = block(self.out_3 + self.out_inv4, self.out_inv3, 3, norm_fn=norm_fn, padding=1, indice_key='res3')
        self.inv_conv3 = block(self.out_inv3, self.out_inv3, 3, norm_fn=norm_fn, indice_key='spconv3', conv_type='inverseconv')

        # [1600, 1408, 41] <- [800, 704, 21]
        # x_conv2, above
        self.out_inv2 = 16
        self.conv_up_t2 = SparseBasicBlock(self.out_2, self.out_2, indice_key='res2', norm_fn=norm_fn)
        self.conv_up_m2 = block(self.out_2 + self.out_inv3, self.out_inv2, 3, norm_fn=norm_fn, indice_key='res2')
        self.inv_conv2 = block(self.out_inv2, self.out_inv2, 3, norm_fn=norm_fn, indice_key='spconv2', conv_type='inverseconv')

        # [1600, 1408, 41] <- [1600, 1408, 41]
        self.out_inv1 = 16
        self.conv_up_t1 = SparseBasicBlock(self.out_1, self.out_1, indice_key='res1', norm_fn=norm_fn)
        self.conv_up_m1 = block(self.out_1 + self.out_inv2, self.out_inv1, 3, norm_fn=norm_fn, indice_key='res1')

        self.conv5 = spconv.SparseSequential(
            block(self.out_inv1, self.out_inv1, 3, norm_fn=norm_fn, padding=1, indice_key='subm1')
        )
        self.num_point_features = 16
    ...
    def forward(self, batch_dict):
        ...
        voxel_features, voxel_coords = batch_dict['voxel_features'], batch_dict['voxel_coords']
        batch_size = batch_dict['batch_size']
        # print(voxel_coords.shape, voxel_coords.device)
        input_sp_tensor = spconv.SparseConvTensor(
            features=voxel_features,
            indices=voxel_coords.int(),
            spatial_shape=self.sparse_shape,
