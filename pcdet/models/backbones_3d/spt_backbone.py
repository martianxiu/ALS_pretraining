...
class SSTInputLayer(nn.Module):
    ...
    def window_partition(self, coors, grid_size):
        ...
        # create coordinsates and windown indices for each shifting. 
        for i in range(2): # 0: no shift. 1: shfit 
            # batch_win_inds (M,): points in a same window is grouped
            # coors_in_win (M, 3)[M, z,y,x]: coords relative to each window 
    ...
    def drop_single_shift(self, batch_win_inds):
        ...
        # assign unique ids to each point within a window. 
        inner_win_inds = sst_ops_utils.get_inner_win_inds(batch_win_inds) 

        bincount = torch.bincount(batch_win_inds) # count points in each window
        num_per_voxel_before_drop = bincount[batch_win_inds] # num_voxel_in_window scattered to each point
        target_num_per_voxel = torch.zeros_like(batch_win_inds)

        for dl in drop_info:
            # for each window, choose a specific number of points based on their number points within. 
            # essentially, perform point sampling within each window
            # for num_voxels in [0,16], take 16
            # for num_voxels in [16,32], take 32
            # for num_voxels in [32,100000], take 64
            ...
            # drop level defined by the number of voxels wihtin each window
            # something like density
            drop_lvl_per_voxel[range_mask] = dl 

        # assert no empty target and all have gone through dropping.
        assert (target_num_per_voxel > 0).all()
        assert (drop_lvl_per_voxel >= 0).all()

        keep_mask = inner_win_inds < target_num_per_voxel # downsampling mask
    ...
    def drop_voxel(self, voxel_info, num_shifts):
        ...
        batch_win_inds_s0 = voxel_info['batch_win_inds_shift0'] # a hash map for each window that groups points within it.
        ...
        # mask to identify kept token based on sampling results within each window
        # drop_levl indicates in which level the points are dropped. ~= density
        keep_mask_s0, drop_lvl_s0 = self.drop_single_shift(batch_win_inds_s0)

        # downsampling of original inds
        ...
        # TODO why?: mask is now keep_mask_s0 & keep_mask_s1
        # voxels are the same for s0 and s1 after dropping?
    ...
    def forward(self, input_dict):
        voxel_features = input_dict['voxel_features'] # (M, C)
        voxel_coords = input_dict['voxel_coords'].long() # M,4
        voxel_shuffle_inds = input_dict['voxel_shuffle_inds'] # M,
        grid_size = input_dict['grid_size'] # grid size in (X,Y,1)
        ...
        # downsample points within each voxel. 
        # points are kept if #tokens is small, and downsampled if its large.
        voxel_info = self.drop_voxel(voxel_info, 2) # voxel_info is updated in this function
        
        # import pdb; pdb.set_trace()
        for i in range(2):
            #TODO: likely getting contiguous window inds and split them based on drop level for per drop level grouping
            ...
            #TODO: likely getting position embedding for each points within each window 
            ...
            #TODO: likely creating a mask to idnetify the valid tokens 
    ...
    def get_pos_embed(self, inds_dict, coors_in_win, feat_dim):
        ...
        # pos_embed_2d = pos_embed_2d.to(dtype) # in case amp, 
...
class SSTBlockV1(nn.Module):
    ...
    def encoder_forward(self, voxel_features, voxel_coords, grid_size):
        ...
        preprocess_dict = {
            ...
            'voxel_coords': voxel_coords, # M, 4
            'voxel_shuffle_inds': voxel_shuffle_inds, # M
            'grid_size': grid_size # (X,Y,1)
        }
        # perform downsampling of voxels wihtin each window and prepare vatious inds 
        ...
        for i, block in enumerate(self.encoder_blocks):
            # feature dtype changed. it is a problem when AMP, float16 changes to float32
            output = block(
                output, pos_embed_list, ind_dict_list, padding_mask_list
            ) # perform windown attention. 
        voxel_features = output

        return voxel_features, voxel_coords, voxel_shuffle_inds

    def forward(self, sp_tensor):
        if self.conv_down is not None: # downsample
            sp_tensor = self.conv_down(sp_tensor)
        voxel_features, voxel_coords, grid_size = \
            self.decouple_sp_tensor(sp_tensor) # (M, C), (M, 4)(bs_idx, 0, y, x), (X,Y,1)
        ...
        # voxel_features_shuffle = voxel_features_shuffle.to(voxel_features.dtype) # in case amp, float32 to float16
        # TODO: though downsampled in the encoder_forward, it seems that all points are kept. 
        # likely, downsampled points are only used for window attention purpose.
...
class SPTBackbone(nn.Module):
    ...
    def forward(self, batch_dict):
        voxel_features, voxel_coords = batch_dict['voxel_features'], batch_dict['voxel_coords'] # (M, C), (M, 4)
        batch_size = batch_dict['batch_size']
        assert torch.all(voxel_coords[:, 1] == 0) # all z = 0. Pillar.
        input_sp_tensor = spconv.SparseConvTensor(
            features=voxel_features,
            indices=voxel_coords[:, [0, 2, 3]].contiguous().int(),  # (bs_idx, y_idx, x_idx)
            spatial_shape=self.sparse_shape,
            batch_size=batch_size
        ) # sparse 2D tensor (y, x). pillar. 
        ...
        # each feature directly upsampled to the output resolution.  
        for i, src in enumerate(self.model_cfg.FEATURES_SOURCE):
            ...
            spatial_features.append(self.deblocks[i](per_features.view(B, -1, Y, X))) # upsampling by transposed 2d conv
            spatial_features_stride.append(multi_scale_3d_strides[src] // self.model_cfg.FUSE_LAYER[src].UPSAMPLE_STRIDE)
        # concat all upsampled features and conv. 
        ...
        batch_dict['multi_scale_3d_features'] = multi_scale_3d_features # encoder features in a dict
        batch_dict['multi_scale_3d_strides'] = multi_scale_3d_strides
        batch_dict['spatial_features'] = spatial_features # upsampled features 
