...
class SPTBackboneMAE(nn.Module):
    ...
    def target_assigner(self, batch_dict):
        ...
        # in real world centers 
        # voxel_centers = common_utils.get_voxel_centers(
        #     voxel_coords[:, 1:], 1, self.voxel_size, self.point_cloud_range
        # )  # (N, 3)

        # in real world centers w/o Z translation. 
        voxel_centers = common_utils.get_voxel_centers_xy(
            voxel_coords[:, 1:], 1, self.voxel_size, self.point_cloud_range
        )  # (N, 3)
    ...
    #     # to voxel space 
    #     norm_gt_points = norm_gt_points / torch.FloatTensor(self.voxel_size).to(norm_gt_points.device)
    ...
    def get_loss(self, tb_dict=None):
        ...
        # gt_points = gt_points.to(pred_points.dtype)
        
        # debug 
        # torch.save(self.forward_ret_dict, "temp/forward_ret_dict.pt")
        # import sys 
        # sys.exit()
        # debug
    ...
    def forward(self, batch_dict):
        all_voxel_features, all_voxel_coords = \
            batch_dict['voxel_features'], batch_dict['voxel_coords'] # (M, C), (M, 4)
        batch_size = batch_dict['batch_size']
        assert torch.all(all_voxel_coords[:, 1] == 0) # all z = 0. Pillar.

        voxel_mae_mask = []
        for bs_idx in range(batch_size):
            voxel_mae_mask.append(
                common_utils.random_masking(1, (all_voxel_coords[:, 0] == bs_idx).sum().item(), self.mask_ratio, all_voxel_coords.device)[0]
            ) # (1, num_voxel). 0: keep, 1: remove
        voxel_mae_mask = torch.cat(voxel_mae_mask, dim=0) # b,num_voxel
        ...
        ) # kept sparse 2D tensor (y, x). pillar. 
        ...
        slices = [all_voxel_coords[:, i].long() for i in [0, 2, 3]] # [batch_idx, y coord, x coord]. indices of orignal voxels
        all_pyramid_voxel_features = spatial_features.permute(0, 2, 3, 1)[slices] # Sparse extraction: extract pixels that correspond to original voxels.  

        target_dict = {
            'voxel_features': all_pyramid_voxel_features, # (M, C)[M, C]
