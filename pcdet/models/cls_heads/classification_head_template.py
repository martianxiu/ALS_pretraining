...
class ClassificationHeadTemplate(nn.Module):
    ...
    def get_cls_layer_loss(self, tb_dict=None):
        cls_labels = self.forward_ret_dict['labels'].view(-1)
        cls_preds = self.forward_ret_dict['logits'].view(-1, self.num_class)
        
        cls_loss = self.cls_loss_func(cls_preds, cls_labels, ignore_index=self.ignore_index)
