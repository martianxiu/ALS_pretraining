...
def get_window_coors(coors, sparse_shape, window_shape, do_shift):
    ...
    win_shape_x, win_shape_y, win_shape_z = window_shape # (8,8,1) hyperparams

    sparse_shape_x, sparse_shape_y, sparse_shape_z = sparse_shape # (X,Y,1)

    # number of windows per dimension. +1 cos shifting will shift 1. 
    ...
    if do_shift: # if no shift, a shift returns cells covering the same areas w/o shifting 
        # when shifting windown ops, window shifts half, within window points group changes
        shift_x, shift_y, shift_z = win_shape_x // 2, win_shape_y // 2, win_shape_z // 2
    else:
        # when no shifting, within window point groups dont change
        shift_x, shift_y, shift_z = win_shape_x, win_shape_y, win_shape_z
    
    # compatibility between 2D window and 3D window
    # z dont shift because pillar. 
    if sparse_shape_z == win_shape_z:
        shift_z = 0

    # coordinate shift relative to windows 
    shifted_coors_x = coors[:, 3] + shift_x
    shifted_coors_y = coors[:, 2] + shift_y
    shifted_coors_z = coors[:, 1] + shift_z

    # get window coordinates for each point
    ...
    assert (win_coors_z == 0).all() # pillar 

    # group points within each local window
    # separate batch, group x, group y, group z. 
    # Each point within a window has a unique identifier.
    # a hashing?  
    ...
    # create coords relative to each window.  
