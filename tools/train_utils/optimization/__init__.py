...
def build_optimizer(model, optim_cfg):
    ...
    elif optim_cfg.OPTIMIZER in ['adamW_onecycle','adamW_cosineanneal']:
        def children(m: nn.Module):
            return list(m.children())
...
def build_scheduler(optimizer, total_iters_each_epoch, total_epochs, last_epoch, optim_cfg):
    ...
    if optim_cfg.OPTIMIZER == 'adam_onecycle' or optim_cfg.OPTIMIZER == 'adamW_onecycle':
        # one cycle: first incresae from LR/10 to LR until pc_start * epoch, then decrases from LR to LR/1e4
        lr_scheduler = OneCycle(
            optimizer, total_steps, optim_cfg.LR, list(optim_cfg.MOMS), optim_cfg.DIV_FACTOR, optim_cfg.PCT_START
        )
    elif optim_cfg.OPTIMIZER == 'adam_cosineanneal' or optim_cfg.OPTIMIZER == 'adamW_cosineanneal':
