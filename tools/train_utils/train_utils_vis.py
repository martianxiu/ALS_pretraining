...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    ...
                    total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, show_gpu_stat=True, use_amp=False, vis_save_dir=None):
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp) and torch.no_grad():
            vis_save_path = join(vis_save_dir, f'{cur_it}.pt')
            loss, tb_dict, disp_dict = model_func(model, batch, vis_save_path)
...
def train_val_model(model, optimizer, train_loader, val_loader, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, vis_save_dir, train_sampler=None,
    ...
    start_epoch = 0
    total_epochs = 1 # for visualization purpose.
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            accumulated_iter = train_one_epoch(
                ...
                use_amp=use_amp,
                vis_save_dir=vis_save_dir # the flag to activate visualization mode, which saves featuers for visualization
            )
